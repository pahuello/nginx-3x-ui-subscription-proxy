...
Обновленная конфигурация обратного прокси-сервера Nginx для объединения подписок с нескольких серверов [3x-UI](https://github.com/MHSanaei/3x-ui?tab=readme-ov-file) в одну.

🇺🇸 [Английская версия README](README.md)

🇷🇺 [Оригинальный русский README](README_RU_ORIGINAL.md)

## Изменения

Добавлена статическая информация о подписке для x-ray клиентов:
+ Заголовок подписки
+ Анонс подписки
+ Интервал обновления
+ Включение/отключение маршрутизации
...
| `PROFILE_TITLE`           | Заголовок подписки                                            |
| `ANNOUNCE`                | Анонс кля клиентов                                            |
| `PROFILE_UPDATE_INTERVAL` | Интервал обновления подписки (например, `12`).                                             |
| `ROUTING_ENABLE`          | Включить/выключить маршрутизацию на клиенте (например, `true` / `yes` / `1` or `false` / `no` / `0`)                                             |
| `ROUTING_RULES`           | ссылка с правилами маршрутизации (например, `happ://routing/add/...`)                                             |
